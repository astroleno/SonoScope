...
export default function HomePage() {
  ...
  const [preset, setPreset] = useState<'pulse' | 'accretion' | 'spiral'>('pulse');
  const [accretionCtrl, setAccretionCtrl] = useState({
    sensMin: 0.92,
    sensMax: 1.18,
    gainScale: 1.15,
    flickerStrength: 0.10,
    flickerFreq: 14,
    overallBoost: 1.10,
  });
  const [frequencyBars, setFrequencyBars] = useState<number[]>([]); // legacy, not used now
  const [features, setFeatures] = useState<{ rms?: number; spectralCentroid?: number; zcr?: number; mfcc?: number[]; spectralFlatness?: number; spectralFlux?: number } | null>(null);
  ...
  const handleStart = async () => {
    try {
      ...
      const provisionalStream = await navigator.mediaDevices.getUserMedia({ audio: true });
      ...
      // 初始化 Meyda 特征提取
      try {
        if (Meyda && Meyda.isBrowser) {
          meydaAnalyzerRef.current = Meyda.createMeydaAnalyzer({
            audioContext,
            source: microphone,
            bufferSize: 1024,
            featureExtractors: ['rms', 'spectralCentroid', 'zcr', 'mfcc', 'spectralFlatness', 'spectralFlux'],
            callback: (f: any) => {
              try {
                setFeatures({
                  rms: typeof f.rms === 'number' ? f.rms : undefined,
                  spectralCentroid: typeof f.spectralCentroid === 'number' ? f.spectralCentroid : undefined,
                  zcr: typeof f.zcr === 'number' ? f.zcr : undefined,
                  mfcc: Array.isArray(f.mfcc) ? f.mfcc : undefined,
                  spectralFlatness: typeof f.spectralFlatness === 'number' ? f.spectralFlatness : undefined,
                  spectralFlux: typeof f.spectralFlux === 'number' ? f.spectralFlux : undefined,
                });
              } catch (e) {
      ...
      try {
        if (testMode) {
          ...
          if (autoCalibrate) {
            ...
            const adj = Math.max(0, normalizedLevel - (noiseFloorRef.current + margin));
          ...
          if (maxAbs < 0.01 && normalizedLevel < 0.005) {
            if (!(window as any).__lastQuietLog || Date.now() - (window as any).__lastQuietLog > 2000) {
              (window as any).__lastQuietLog = Date.now();
              console.log('静音区间调试: maxAbs=', maxAbs.toFixed(4), 'RMS=', normalizedLevel.toFixed(4), 'min=', minVal.toFixed(3), 'max=', maxVal.toFixed(3));
            }
          }
        }
    ...
    <main className="min-h-screen bg-black text-white flex flex-col items-center justify-center relative">
      ...
      {isStarted && preset === 'accretion' && (
        <div className="mt-4 z-10 w-[min(92vw,680px)] bg-white/5 border border-white/10 rounded-lg p-4 text-sm">
          <div className="mb-2 text-gray-300 font-semibold">Accretion 控制面板</div>
          <div className="grid md:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs text-gray-400 mb-1">灵敏度下限 sensMin ({accretionCtrl.sensMin.toFixed(2)})</label>
              <input type="range" min={0.85} max={1.1} step={0.01} value={accretionCtrl.sensMin}
                onChange={(e) => setAccretionCtrl(v => ({ ...v, sensMin: Math.min(parseFloat(e.target.value), v.sensMax - 0.01) }))}
                className="w-full" />
            </div>
            <div>
              <label className="block text-xs text-gray-400 mb-1">灵敏度上限 sensMax ({accretionCtrl.sensMax.toFixed(2)})</label>
              <input type="range" min={1.0} max={1.5} step={0.01} value={accretionCtrl.sensMax}
                onChange={(e) => setAccretionCtrl(v => ({ ...v, sensMax: Math.max(parseFloat(e.target.value), v.sensMin + 0.01) }))}
                className="w-full" />
            </div>
            <div>
              <label className="block text-xs text-gray-400 mb-1">增益比例 gainScale ({accretionCtrl.gainScale.toFixed(2)})</label>
              <input type="range" min={0.6} max={1.8} step={0.02} value={accretionCtrl.gainScale}
                onChange={(e) => setAccretionCtrl(v => ({ ...v, gainScale: parseFloat(e.target.value) }))}
                className="w-full" />
            </div>
            <div>
              <label className="block text-xs text-gray-400 mb-1">总体亮度 overallBoost ({(accretionCtrl as any).overallBoost?.toFixed(2) ?? '1.00'})</label>
              <input type="range" min={0.7} max={1.6} step={0.02} value={(accretionCtrl as any).overallBoost ?? 1.0}
                onChange={(e) => setAccretionCtrl(v => ({ ...v, overallBoost: parseFloat(e.target.value) }))}
                className="w-full" />
            </div>
            <div>
              <label className="block text-xs text-gray-400 mb-1">闪烁强度 flickerStrength ({accretionCtrl.flickerStrength.toFixed(2)})</label>
              <input type="range" min={0.0} max={0.35} step={0.01} value={accretionCtrl.flickerStrength}
                onChange={(e) => setAccretionCtrl(v => ({ ...v, flickerStrength: parseFloat(e.target.value) }))}
                className="w-full" />
            </div>
            <div className="md:col-span-2">
              <label className="block text-xs text-gray-400 mb-1">闪烁频率 flickerFreq ({accretionCtrl.flickerFreq.toFixed(0)})</label>
              <input type="range" min={6} max={32} step={1} value={accretionCtrl.flickerFreq}
                onChange={(e) => setAccretionCtrl(v => ({ ...v, flickerFreq: parseFloat(e.target.value) }))}
                className="w-full" />
            </div>
          </div>
        </div>
        ...
        <div className="mb-4 z-10 px-4 py-3 bg-white/5 border border-white/10 rounded-lg text-sm text-gray-200">
          <div className="flex items-center gap-3 flex-wrap">
            <div className="flex items-center gap-2">
              <span className={`inline-block w-2.5 h-2.5 rounded-full ${signalOn ? 'bg-green-400' : 'bg-gray-500'}`}></span>
            ...
            <div className="flex items-center gap-2 text-gray-400">
              <span>轨道:</span>
              <span>{streamRef.current && streamRef.current.getAudioTracks().length > 0 ? (streamRef.current.getAudioTracks()[0].muted ? '已静音' : '活动') : '未连接'}</span>
            </div>
          </div>
        </div>
      )}
      ...
      {isStarted && (
        <div className="mb-6 z-10 w-64">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-sm text-gray-400">{Math.round(audioLevel * 100)}%</span>
          </div>
      ...
      <div className="flex gap-4 z-10">
        ...
        <select
          value={preset}
          onChange={(e) => setPreset(e.target.value as 'pulse' | 'accretion' | 'spiral')}
      ...
      <div className="mt-8 max-w-lg text-center text-gray-400 z-10">
        <p className="text-sm">
          点击"开始"按钮授权麦克风访问，然后播放音乐或发出声音来体验实时可视化效果
        </p>
        {!isStarted && (
      ...
      <Visualizer 
        key={preset}
        audioLevel={testMode ? (Math.sin(Date.now() * 0.01) * 0.5 + 0.5) : audioLevel}
        running={isStarted}
        preset={preset}
        features={features}
