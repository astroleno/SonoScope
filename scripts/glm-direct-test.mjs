...
async function callGLM(prompt, url, apiKey, need){
  const body = {
    model: 'glm-4.5-air',
    messages: [
...
async function main(){
  // 读取环境变量/配置
  let env = {};
  try { env = JSON.parse(fs.readFileSync('./env.local.json','utf8')); } catch {}
  ...
  if (!apiKey) { console.error('缺少 ZHIPU_API_KEY 或 NEXT_PUBLIC_GLM_API_KEY'); process.exit(1); }

  // 测试向量（不依赖 mock 框架，仅作为输入示例）
  const features = {
    tempo_bpm: 128,
    beat_strength: 0.78,
    voiceProb_mean: 0.12,
  ...
  await callGLM(prompt, url, apiKey, need);
}
